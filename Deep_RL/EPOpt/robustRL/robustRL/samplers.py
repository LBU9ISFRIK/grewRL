...
def sample_paths(N,
    ...
    for ep in range(N):
        ...
        if mujoco_env == True:
            if normalized_env:
                ...
                qpos.append(env.env.data.qpos.reshape(-1))
                qvel.append(env.env.data.qvel.reshape(-1))
        done = False
        t = 0
    ...
    #print "Env body    _mass: ", env.env.model.body_mass[1]
...
def sample_paths_parallel(N,
    ...
    if num_cpu == None or num_cpu == 'max':
        ...
        return sample_paths(N, policy, baseline, env_mode, T, gamma, mujoco_env, normalized_env)
    else:
