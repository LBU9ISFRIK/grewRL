...
[InitializeOnLoad]
#if UNITY_2018_2_OR_NEWER
public class ModelLoaderCheckPlugins : IPreprocessBuildWithReport, IPostprocessBuildWithReport
#else
public class ModelLoaderCheckPlugins : IPreprocessBuild, IPostprocessBuild
#endif
{
    public const string DebugSymbol = "ModelLoader_OUTPUT_MESSAGES";
    public const string ZipSymbol = "ModelLoader_USE_ZIP";
    public const string DebugEnabledMenuPath = "ModelLoader/Enable Debug";
    public const string ZipEnabledMenuPath = "ModelLoader/Enable Zip loading";
#if UNITY_EDITOR_OSX
	public const string XCodeProjectPath = "Libraries/ModelLoader/ModelLoader/Plugins/iOS";
#endif
    ...
    static ModelLoaderCheckPlugins()
    {
        try
        {
            CheckForOldVersions();
                ...
                if (assimpInteropAssetPath.EndsWith("AssimpInterop.cs"))
                {
                    ...
                    if (scriptsDirectory != null && scriptsDirectory.Parent != null)
                    {
                        ...
                        if (pluginsDirectories.Length > 0)
                        {
                            ...
                            {
                                EditorUtility.DisplayDialog("ModelLoader", "Looks like you have an old ModelLoader install mixed with the newest update.\nPlease make a clean ModelLoader install (remove all ModelLoader files and install ModelLoader again).", "Ok");
                            }
                        }
                    }
                }
    ...
    public void OnPreprocessBuild(BuildTarget target, string path)
    {
        ...
        if (buildTarget == BuildTarget.WebGL)
        {
            ...
            var pluginsLabel = "_ModelLoader_Emscripten_1.38.11_";
#elif UNITY_2018_3_OR_NEWER
            var pluginsLabel = "_ModelLoader_Emscripten_1.37.40_";
#elif UNITY_2018_2_OR_NEWER
            var pluginsLabel = "_ModelLoader_Emscripten_1.37.33_";
#elif UNITY_2017_4_OR_NEWER
            var pluginsLabel = "_ModelLoader_Emscripten_1.37.3_";
#else
			throw new Exception ("Your Unity WebGL version is not supported by ModelLoader.");
#endif
            ...
            foreach (var importer in webGLImporters)
            {
                if (importer.isNativePlugin)
                {
                    ...
                    if (labels.Contains("_ModelLoader_WebGL_"))
                    {
        ...
        {
            var pluginsLabel = EditorUtility.DisplayDialog("ModelLoader", "Building to iOS Device (iPhone, iPad) or iOS Simulator?", "Device", "Simulator") ? "_ModelLoader_iOS_Device_" : "_ModelLoader_iOS_Simulator_";
#if UNITY_EDITOR_OSX
		    _iosFileSharingEnabled = EditorUtility.DisplayDialog ("ModelLoader", "Enable iOS File Sharing?", "Yes", "No");
#endif
            ...
            foreach (var importer in iOSImporters)
            {
                if (importer.isNativePlugin)
                {
                    var asset = AssetDatabase.LoadMainAssetAtPath(importer.assetPath);
                    var labels = AssetDatabase.GetLabels(asset);
                    if (labels.Contains("_ModelLoader_iOS_"))
                    {
                        importer.SetIncludeInBuildDelegate(assetPath => labels.Contains(pluginsLabel));
                    }
                }
...
#if UNITY_EDITOR_OSX
            EditorUtility.DisplayDialog("ModelLoader", "Warning: Bitcode is not supported by ModelLoader and will be disabled.", "Ok");
#else
            EditorUtility.DisplayDialog("ModelLoader", "Warning:\nBitcode is not supported. You should disable it on your project settings.\nZLIB library should be included in your project frameworks.", "Ok");
#endif
        }
#if !UNITY_2018_2_OR_NEWER
        else if (buildTarget == BuildTarget.Android)
        {
            ...
            foreach (var importer in androidImporters)
            {
                if (importer.isNativePlugin)
                {
                    var asset = AssetDatabase.LoadMainAssetAtPath(importer.assetPath);
                    var labels = AssetDatabase.GetLabels(asset);
                    if (labels.Contains("_ModelLoader_Android_") && labels.Contains("_ModelLoader_Android_ARM64_"))
                    {
        ...
        foreach (var importer in allImporters)
        {
            if (!importer.isNativePlugin)
            {
                var asset = AssetDatabase.LoadMainAssetAtPath(importer.assetPath);
                var labels = AssetDatabase.GetLabels(asset);
                if (labels.Contains("_ModelLoader_ZIP_"))
                {
#if ModelLoader_USE_ZIP
