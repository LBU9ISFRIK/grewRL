...
def AC(env_name, hidden_sizes=[32], ac_lr=5e-3, cr_lr=8e-3, num_epochs=50, gamma=0.99, steps_per_epoch=100,  steps_to_print=100, action_type = 'Discrete'):
    '''
...
s
    ...
    if action_type == 'Discrete':
        ...
        low_action_space = env.action_space.low
        high_action_space = env.action_space.high
    ...
    file_writer = tf.summary.FileWriter('log_dir/{}/AC_{}_{}'.format(env_name, clock_time, hyp_str),
    ...
    for ep in range(num_epochs):
        ...
        old_p_loss, old_v_loss, epochs_summary = sess.run([p_loss, v_loss, pre_scalar_summary],
                                                          feed_dict={obs_ph: obs_batch, act_ph: act_batch,
                                                                     ret_ph: ret_batch, rtg_ph: rtg_batch})
        ...
        new_p_loss, new_v_loss, train_summary_run = sess.run([p_loss, v_loss, train_summary],
                                                             feed_dict={obs_ph: obs_batch, act_ph: act_batch,
                                                                        ret_ph: ret_batch, rtg_ph: rtg_batch})
        ...
        if last_print_step > steps_to_print:
            ...
            ep, np.mean(train_rewards), np.max(train_rewards), np.mean(train_ep_len), len(buffer), step_count,
            time.time() - timer))
...
if __name__ == '__main__':
    AC('Ant-v2', hidden_sizes=[64], ac_lr=4e-3, cr_lr=1.5e-2, gamma=0.99, steps_per_epoch=100,   steps_to_print=5000, num_epochs=8000, action_type = 'Box')