# -*- coding: utf-8 -*-
"""
Created on Mon Aug 31 11:11:10 2020
...
def cnn(x):
    ...
    x = tf.layers.conv2d(x, filters=32, kernel_size=8, strides=2, padding='valid', activation='relu')
...
def qnet(x, hidden_layers, output_size, fnn_activation=tf.nn.relu, last_activation=None):
    ...
    # deep Q network: CMM followed by FNN
    x = cnn(x)
...
class ExperienceBuffer():
    ...
    def __init__(self, buffer_size):
        ...
        self.act_buffer = deque(maxlen=buffer_size)
    ...
    def sample_minibatch(self, batch_size):
        ...
        mb_indices = np.random.randint(len(self.obs_buffer), size=batch_size)

        mb_obs = scale_frames([self.obs_buffer[i] for i in mb_indices])
        mb_rew = [self.rew_buffer[i] for i in mb_indices]
        mb_act = [self.act_buffer[i] for i in mb_indices]
        mb_obs2 = scale_frames([self.obs2_buffer[i] for i in mb_indices])
        mb_done = [self.done_buffer[i] for i in mb_indices]

        return mb_obs, mb_rew, mb_act, mb_obs2, mb_obs2

    def __len__(self):
        return len(self.obs_buffer)

def q_target_values(mini_batcj_rw, mini_batch_done, av, discounted_value):
    ...
    for r, d, av in zip(mini_batcj_rw, mini_batch_done, max_av):
        if d:
            ys.append(r)
        else:
            q_step = r + discounted_value * av
            ys.append(q_step)
    assert len(ys) == len(mini_batcj_rw)
    return ys
...
def DQN(env_name, hidden_sizes=[32], lr=1e-2, num_epochs=2000, buffer_size=100000, discount=0.99, render_cycle=100,
        update_target_net=1000,
        batch_size=64, update_freq=4, frames_num=2, min_buffer_size=5000, test_frequency=20, start_explor=1,
        end_explor=0.1, explor_steps=100000):
    ...
    update_target = [train_vars[i].assign(train_vars[i + len(target_vars)]) for i in
                     range(len(train_vars) - len(target_vars))]
    ...
    for ep in range(num_epochs):
        ...
        while not done:
            ...
            if len(buffer) > min_buffer_size and (step_count % update_freq == 0):
                ...
                train_summary, train_loss, _ = sess.run([scalar_summary, v_loss, v_opt],
            ...
            if (len(buffer) > min_buffer_size) and (step_count % update_target_net == 0):
                ...
                _, train_summary = sess.run([update_target_op, mean_loss_summary],
        ...
        if ep % test_frequency == 0:
            ...
            print('Ep:%4d Rew:%4.2f, Eps:%2.2f -- Step:%5d -- Test:%4.2f %4.2f -- Time:%d -- Ep_Steps:%d' %
                  (ep, np.mean(batch_rew), eps, step_count, np.mean(test_rw), np.std(test_rw), ep_sec_time,
